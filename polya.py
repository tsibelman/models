...
from functions import read_csv, draw_chart # импорт функции read_csv из файла functions.py
...
def production_forecast(rig_ratio, extrapolation_range, ignore_productivity) : #      рассчитывает
    ...
    #input_length = len(rig_count)     # переменная для обращения к последнему элементу массива
...
production2, new_production2, old_production_decline2, rig_count2, rig_productivity2 = production_forecast(0, 5, False)

a=[0]

#draw_chart(production1, production2, 'chart.png')
draw_chart(a, production2, 'chart.png')
#draw_chart(new_production1, old_production_decline1, 'chart2.png')
draw_chart(new_production2, old_production_decline2, 'chart3.png')
